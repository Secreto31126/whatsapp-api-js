...
export interface API {
    ...
    /**
     * Send a Whatsapp message to a specific recipient type,
     * which can be either a phone number, a BSUID, or a group ID.
     *
     * @example
     * ```ts
     * import { WhatsAppAPI } from "whatsapp-api-js";
     ...
     *
     * const phoneID = "YOUR_BOT_NUMBER";
     * const users = [{ phone: "YOUR_USER1_NUMBER" }, { bsuid: "YOUR_USER2_BSUID" }];
     * const message = new Text("Hello World");
     *
     * const responses = Whatsapp.broadcastMessage(phoneID, users, message);
     *
     ...
     *
     * const phoneID = "YOUR_BOT_NUMBER";
     * const users = [{ key: "USER1_ID" }, { key: "USER2_ID" }];
     * const message_builder = ({ key }) => [{ bsuid: DB.fetch(key).phone_id }, new Text(`#${key}`)];
     *
     * const responses = Whatsapp.broadcastMessage(phoneID, users, message);
     *
     * Promise.all(responses).then(console.log);
     ...
     * @throws if delay is lower than 0
     */
    broadcastMessage<T>(phoneID: string, recipients: T[], message_builder: (data: T) => [ClientRecipientIdentifier, ClientMessage], batch_size: number, delay: number): Array<ReturnType<API["sendMessage"]>>;
    /**
