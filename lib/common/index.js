...
class WhatsAppAPI {
     ...
     */
    constructor({ token, appSecret, webhookVerifyToken, v = "v16.0", parsed = true, secure = true, ponyfill = {} }) {
        this.token = token;
        this.secure = !!secure;
        if (this.secure) {
            this.appSecret = appSecret;
            if (typeof ponyfill.subtle !== "object" &&
                ...
                throw new Error("subtle is not defined in the enviroment, please provide a valid ponyfill object with the parameter 'ponyfill.subtle'.");
            }
        ...
        if (typeof ponyfill.fetch !== "function" &&
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-ignore - fetch might not be defined in the enviroment
            typeof fetch !== "function") {
            throw new Error("fetch is not defined in the enviroment, please provide a valid ponyfill function with the parameter 'ponyfill.fetch'.");
        }
        // Let's hope the user is using a valid ponyfill
        this.fetch =
            ponyfill.fetch ||
     ...
     */
    async sendMessage(phoneID, to, message, context) {
        ...
        request[type] =
            ...
            message._build();
        if (context)
            request.context = { message_id: context };
        // Make the post request
        const promise = this.fetch(`https://graph.facebook.com/${this.v}/${phoneID}/messages`, {
            method: "POST",
            headers: {
     ...
     *
     */
    async broadcastMessage(phoneID, to, message, batch_size = 50, delay = 1000) {
        const responses = [];
        if (batch_size < 1) {
    ...
    async createQR(phoneID, message, format = "png") {
        const promise = this.fetch(`https://graph.facebook.com/${this.v}/${phoneID}/message_qrdls?generate_qr_image=${format}&prefilled_message=${message}`, {
            method: "POST",
            headers: {
                Authorization: `Bearer ${this.token}`
            }
    ...
    async updateQR(phoneID, id, message) {
        const promise = this.fetch(`https://graph.facebook.com/${this.v}/${phoneID}/message_qrdls/${id}?prefilled_message=${message}`, {
            method: "POST",
            headers: {
                Authorization: `Bearer ${this.token}`
            }
    ...
    async retrieveMedia(id, phoneID) {
        ...
        const promise = this.fetch(`https://graph.facebook.com/${this.v}/${id}?${params}`, {
            headers: {
                Authorization: `Bearer ${this.token}`
            }
        });
    ...
    async uploadMedia(phoneID, form, check = true) {
        if (check) {
            if (!form ||
                ...
                throw new TypeError("File's Form must be an instance of FormData");
            ...
            if (file.size && file.size > validMediaSizes[mediaType])
                throw new Error(`File is too big (${file.size} bytes) for a ${mediaType} (${validMediaSizes[mediaType]} bytes limit)`);
        }
        const promise = this.fetch(`https://graph.facebook.com/${this.v}/${phoneID}/media?messaging_product=whatsapp`, {
            method: "POST",
            body: form,
            headers: {
                Authorization: `Bearer ${this.token}`,
    ...
    async post(data, raw_body, signature) {
        ...
        if (this.secure) {
            ...
            const key = await this.subtle.importKey("raw", keyBuffer, { name: "HMAC", hash: "SHA-256" }, true, ["sign", "verify"]);
