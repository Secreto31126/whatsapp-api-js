...
class WhatsAppAPI {
  ...
  constructor({
    ...
    if (v)
      ...
      console.warn(
        `[whatsapp-api-js] Cloud API version not defined. In production, it's strongly recommended pinning it to the desired version with the "v" argument. Defaulting to "${import_types.DEFAULT_API_VERSION}".`
      );
      this.v = import_types.DEFAULT_API_VERSION;
    }
    this.parsed = !!parsed;
   ...
   */
  async broadcastMessage(phoneID, to, message, batch_size = 50, delay = 1e3) {
    const responses = [];
    if (batch_size < 1) {
  ...
  async post(data, raw_body, signature) {
    if (this.secure) {
      ...
      const data2 = encoder.encode((0, import_utils.escapeUnicode)(raw_body));
      const result = await this.subtle.sign("HMAC", key, data2.buffer);
   ...
   */
  _authenticatedRequest(url, headers = {}) {
    if (!url)
