...
class WhatsAppAPI {
  ...
  async sendMessage(phoneID, recipient, message, context, biz_opaque_callback_data) {
    ...
    const request = {
      ...
      type,
      [type]: message,
      recipient: individual ? r.pbsuid ?? r.bsuid : void 0,
      context: context ? { message_id: context } : void 0,
  ...
  async initiateCall(phoneID, callee, sdp, biz_opaque_callback_data) {
  ...
  async createGroup(phoneID, subject, description, joinApprovalMode) {
    const promise = this.$$apiFetch$$(
  ...
  async retrieveGroups(phoneID, limit, after, before) {
    const params = new URLSearchParams();
    if (limit !== void 0) params.set("limit", limit.toString());
  ...
  async retrieveGroup(groupID, fields) {
  ...
  async updateGroup(groupID, settings) {
    const promise = this.$$apiFetch$$(
  ...
  async updateGroupPicture(groupID, form, check = true) {
    if (check) {
      const file = WhatsAppAPI.getFormFile(form);
  ...
  async retrieveGroupInviteLink(groupID) {
    const promise = this.$$apiFetch$$(
      `https://graph.facebook.com/${this.v}/${groupID}/invite_link`
    );
    return this.getBody(promise);
  }
  async resetGroupInviteLink(groupID) {
    const promise = this.$$apiFetch$$(
      `https://graph.facebook.com/${this.v}/${groupID}/invite_link`,
      {
        method: "POST",
  ...
  async removeGroupParticipants(groupID, ...users) {
    if (users.length > 8)
  ...
  async retrieveGroupJoinRequests(groupID) {
    const promise = this.$$apiFetch$$(
      `https://graph.facebook.com/${this.v}/${groupID}/join_requests`
    );
    return this.getBody(promise);
  }
  async approveGroupJoinRequests(groupID, ...requests) {
    const promise = this.$$apiFetch$$(
      `https://graph.facebook.com/${this.v}/${groupID}/join_requests`,
      {
        method: "POST",
  ...
  async rejectGroupJoinRequests(groupID, ...requests) {
    const promise = this.$$apiFetch$$(
      `https://graph.facebook.com/${this.v}/${groupID}/join_requests`,
      {
        method: "DELETE",
   ...
   */
  static toRecipient(r) {
    if (Array.isArray(r)) return r.map(WhatsAppAPI.toRecipient);
    if (typeof r !== "string") return r;
    if (/^[a-zA-Z]{2}\..+/.test(r)) return { bsuid: r };
    return { phone: r };
  }
  static isIndividualRecipient(r) {
